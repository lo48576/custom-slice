...
/*
// I want these to be generated by `new_checked = ...`.

impl StdString {
    fn from_utf8(v: Vec<u8>) -> Result<Self, FromUtf8Error> {
        match validate(<Vec<u8> as std::borrow::Borrow<[u8]>>::borrow(&v)) {
            Ok(_) => Ok(unsafe { Self::from_utf8_unchecked(v) }),
            Err(e) => Err(FromUtf8Error::new(e, v)),
        }
    }
}
